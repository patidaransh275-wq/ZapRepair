...
import { Star, ShieldCheck, MapPin, Clock, ArrowRight, CheckCircle2, ChevronDown, ChevronUp, Zap, HelpCircle } from 'lucide-react';
...
export default function ServiceDetailPage({ params }) {
  ...
  const [pinInput, setPinInput] = useState(userPincode || '110001');
  ...
  if (!service) {
    return (
      <div className="py-24 text-center space-y-4">
        ...
        <p className="text-slate-600">The requested appliance service page could not be found.</p>
        <Link href="/services" className="inline-block bg-slate-900 text-white font-bold px-6 py-2.5 rounded-xl">
          View All Services
        </Link>
      </div>
    );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            ...
            <div className="lg:col-span-7 space-y-4">
              ...
              <div className="flex items-center gap-4 text-xs pt-2">
                ...
                <span className="text-slate-300 font-semibold">{service.reviewCount}+ Doorstep Repairs</span>
                <span className="text-slate-400">•</span>
            ...
            <div className="lg:col-span-5">
              <div className="relative rounded-2xl overflow-hidden shadow-2xl border border-slate-800">
                ...
                <div className="absolute bottom-4 left-4 right-4 text-xs text-white bg-slate-900/90 backdrop-blur-md p-3 rounded-xl border border-slate-700">
                  ⚡ <span className="font-bold text-amber-400">45-Minute Doorstep Arrival</span> guaranteed across all major city pincodes.
                </div>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 relative z-10 space-y-12">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Transparent labor & service charges. Inspection fee waived if repair is approved.
              </p>
            </div>
          </div>
        ...
        {service.issues && service.issues.length > 0 && (
          <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-soft-sm space-y-6">
            <h2 className="text-2xl font-bold text-slate-900 font-heading">
              Common {service.name} Problems Solved
            </h2>
        ...
        <div className="bg-slate-900 text-white rounded-2xl p-8 border border-slate-800 text-center space-y-4">
          <h3 className="text-xl font-bold font-heading">Need Emergency {service.name} Repair?</h3>
          <p className="text-xs text-slate-300 max-w-xl mx-auto">
            Our technicians are active in Delhi NCR, Mumbai, Bangalore, Pune, Hyderabad, Jaipur, and Chennai with fast 45-minute doorstep arrival.
          </p>
          <button
            onClick={() => openBookingModal(service.id)}
